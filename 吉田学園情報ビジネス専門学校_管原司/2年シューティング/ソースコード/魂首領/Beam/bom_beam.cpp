...
// コンストラクタ
//******************************************************************************
CBom_Beam::CBom_Beam(int nPriority) : CScene(nPriority)
{
...
// 生成関数
//******************************************************************************
CBom_Beam * CBom_Beam::Create(D3DXVECTOR3 pos, D3DXVECTOR3 size)
{
	// CBeamクラスのポインタ
	CBom_Beam *pPlayer_Beam;

	// メモリ確保
	pPlayer_Beam = new CBom_Beam;
...
// 描画関数
//******************************************************************************
void CBom_Beam::Draw(void)
{
}
//******************************************************************************
// ビーム生成
//******************************************************************************
void CBom_Beam::BeamCreate(D3DXVECTOR3 pos, D3DXVECTOR3 size)
{
	// ビーム生成
...
// ビーム破棄
//******************************************************************************
void CBom_Beam::ReleaseAllBeam(void)
{
	// 最大数分回す
...
void CBom_Beam::Collision(void)
{
	...
	do
	{
		...
		if (pScene != NULL)
		{
			...
			{
				// 敵の位置
				D3DXVECTOR3 BulletPos = ((CBoss_Left*)pScene)->GetPosition();

				// 敵のサイズ
				D3DXVECTOR3 BulletSize = ((CBoss_Left*)pScene)->GetSize();

				// 代入
				D3DXVECTOR3 box1Max = D3DXVECTOR3(size.x / DEVIDE_VALUE, size.y / DEVIDE_VALUE, size.z / DEVIDE_VALUE) + pos;
				D3DXVECTOR3 box1Min = D3DXVECTOR3(-size.x / DEVIDE_VALUE, -fSize / DEVIDE_VALUE, -size.z / DEVIDE_VALUE) + pos;
				D3DXVECTOR3 box2Max = D3DXVECTOR3(BulletSize.x / DEVIDE_VALUE, BulletSize.y / DEVIDE_VALUE, BulletSize.z / DEVIDE_VALUE) + BulletPos;
				D3DXVECTOR3 box2Min = D3DXVECTOR3(-BulletSize.x / DEVIDE_VALUE, -BulletSize.y / DEVIDE_VALUE, -BulletSize.z / DEVIDE_VALUE) + BulletPos;
				...
				{
					// サイズ代入
					fSize = (pos.y + fSize / DEVIDE_VALUE) - (BulletSize.y + BulletSize.y / DEVIDE_VALUE);
