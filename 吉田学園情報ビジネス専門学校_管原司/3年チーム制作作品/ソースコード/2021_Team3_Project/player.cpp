...
// Author : SugawaraTsukasa
//=============================================================================
void CPlayer::KeyboardAttack(void)
{
	// キーボード取得
	CInputKeyboard *pKeyboard = CManager::GetKeyboard();
	...
	if (m_nAttackCount > ZERO_INT)
	{
		...
		// カウントが60の場合
		if (m_nAttackCount >= ATTACK_COOLTIME)
		{
			// 0に
			m_nAttackCount = ZERO_INT;
		}
...
// Author : SugawaraTsukasa
//=======================================================================================
void CPlayer::CreateWaveRight(void)
{
	// 左の歯車の情報取得
	CModelAnime *pGear_R = GetModelAnime(GEAR_R_NUM);

	// 位置取得
	D3DXVECTOR3 pos = pGear_R->GetMtxPos();
	pos.y -= WAVE_Y;

	// しぶき生成
	// 水しぶき生成
	CParticle_Effect::Create(pos, CParticle_Effect::TYPE_WAVE);
}
//=======================================================================================
// 波生成関数
// Author : SugawaraTsukasa
//=======================================================================================
void CPlayer::CreateWaveLeft(void)
{
	// 左の歯車の情報取得
	CModelAnime *pGear_L = GetModelAnime(GEAR_L_NUM);

	// 位置取得
	D3DXVECTOR3 pos = pGear_L->GetMtxPos();
	pos.y -= WAVE_Y;
